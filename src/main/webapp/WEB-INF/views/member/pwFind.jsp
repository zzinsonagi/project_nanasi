  ...
  <body>
    ...
    <div class="terms">
      <div class="termsTitle">
        <h4>비밀번호 찾기</h4>
        <div class="line"></div>
        <p>인증번호를 받아 비밀번호를 찾을 수 있습니다.</p>
      </div>
    </div>

    <div class="pwFind">
      <form name="pwFind" method="post" action="" id="idFind" onsubmit="return check();">
        <div class="loginInfo">
          <div class="idPw" style="margin: 0">
            ...
            <div class="msg">
              <input class="w3-input" type="text" id="user_id" name="user_id" placeholder="아이디를 입력해주세요." />
              <p id="idmsg"></p>
            </div>
            <div class="memEmail">
          ...
          <div class="idPwBtn">
            <button type="submit" class="btn_ok">비밀번호 찾기</button>
          </div>
        </div>
      </form>
    </div>
    ...
    <script>
      function check() {
        if (pwFind.user_name.value == "") {
          alert("이름을 입력해주세요.");
          pwFind.user_name.focus();
          return false;
        }
        if (pwFind.user_id.value == "") {
          alert("아이디를 입력해주세요.");
          pwFind.user_id.focus();
          return false;
        }
        if (pwFind.user_email.value == "") {
          alert("이메일을 입력해주세요.");
          pwFind.user_email.focus();
          return false;
        }
        if (pwFind.email_chk.value == "") {
          alert("이메일을 인증해주세요.");
          pwFind.email_chk.focus();
          return false;
        }
        return true;
      }
