<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%> <%@ include
file="../main/header.jsp"%>

<!-- ----------------------------------- content ----------------------------------- -->

<div class="title-container">
  <div class="title">
    <h2>My Page</h2>
    <p>나의 구독관리 > 구독 목록</p>
  </div>
</div>

<div class="Mypage">
  <div class="container" style="padding-right: 20px">
    <div class="row" style="padding: 0 20px">
      <div class="col-md-2">
        <div class="line line-category"></div>
        <ul class="navbar-nav pcCate">
          <li><a class="category-big">나의 구독관리</a></li>
          <li><a class="category-small" href="../my/wish.html">위시리스트</a></li>
          <li><a class="category-small" href="../my/subList.html" style="color: #185fff">구독 목록</a></li>
        </ul>
        <ul class="navbar-nav homeCate">
          <li><a class="category-big">회원정보 관리</a></li>
          <li><a class="category-small" href="../my/myInfo.html">내 정보</a></li>
          <li><a class="category-small" href="../my/myinfoEdit.html">회원 정보 수정</a></li>
          <li><a class="category-small" href="../my/pwEdit.html">비밀번호 변경</a></li>
        </ul>
      </div>

      <div class="col-md-10">
        <form name="" method="">
          <!-- 반복되는 부분 -->
          <div class="wishList">
            <div class="wishProductImg">
              <div class="thumbnailImg">
                <img src="../../static/images/michieda/michieda1.png" alt="썸네일 이미지" />
              </div>
            </div>
            <div class="wishProductInfoBtn">
              <div class="wishProductInfo">
                <h4>미치에다 슌스케 (구독중)</h4>
                <p>컬러 : 스타라이트 | 구독기간 : 3개월</p>
                <div class="wishPriceInfo">
                  <span>월 18,000원</span>
                </div>
              </div>
              <div class="wishProductBtn">
                <button type="button" class="wishOption" onclick="return returnRequest()">회수 신청</button>
                <button type="button" class="wishOption" onclick="window.location.href='../my/damageRegister.html'">
                  고장 및 파손 접수
                </button>
                <button type="button" class="wishSub" onclick="window.location.href='../my/reviewWrite.html'">
                  리뷰 작성
                </button>
              </div>
            </div>
          </div>
          <div class="line"></div>
          <!-- 반복되는 부분 끝 -->

          <!-- 고장 및 파손 접수 -->
          <div class="wishList">
            <div class="wishProductImg">
              <div class="thumbnailImg">
                <img src="../../static/images/michieda/michieda1.png" alt="썸네일 이미지" />
              </div>
            </div>
            <div class="wishProductInfoBtn">
              <div class="wishProductInfo">
                <h4>미치에다 슌스케 (고장 및 파손 접수)</h4>
                <p>컬러 : 스타라이트 | 구독기간 : 3개월</p>
                <div class="wishPriceInfo">
                  <span>월 18,000원</span>
                </div>
              </div>
              <div class="wishProductBtn">
                <button type="button" class="btnDisable" disabled>회수 신청</button>
                <button type="button" class="btnDisable" disabled>고장 및 파손 접수</button>
                <button type="button" class="btnDisable" disabled>리뷰 작성</button>
              </div>
            </div>
          </div>
          <div class="line"></div>
          <!-- 고장 및 파손 접수 끝 -->

          <!-- 회수 신청 -->
          <div class="wishList">
            <div class="wishProductImg">
              <div class="thumbnailImg">
                <img src="../../static/images/michieda/michieda1.png" alt="썸네일 이미지" />
              </div>
            </div>
            <div class="wishProductInfoBtn">
              <div class="wishProductInfo">
                <h4>미치에다 슌스케 (회수 신청)</h4>
                <p>컬러 : 스타라이트 | 구독기간 : 3개월</p>
                <div class="wishPriceInfo">
                  <span>월 18,000원</span>
                </div>
              </div>
              <div class="wishProductBtn">
                <button type="button" class="btnDisable" disabled>회수 신청</button>
                <button type="button" class="btnDisable" disabled>고장 및 파손 접수</button>
                <button type="button" class="btnDisable" disabled>리뷰 작성</button>
              </div>
            </div>
          </div>
          <div class="line"></div>
          <!-- 회수 신청 끝 -->

          <!-- 구독 완료 -->
          <div class="wishList">
            <div class="wishProductImg">
              <div class="thumbnailImg">
                <img src="../../static/images/michieda/michieda1.png" alt="썸네일 이미지" />
              </div>
            </div>
            <div class="wishProductInfoBtn">
              <div class="wishProductInfo">
                <h4>미치에다 슌스케 (구독 완료)</h4>
                <p>컬러 : 스타라이트 | 구독기간 : 3개월</p>
                <div class="wishPriceInfo">
                  <span>월 18,000원</span>
                </div>
              </div>
              <div class="wishProductBtn">
                <button type="button" class="btnDisable" disabled>회수 신청</button>
                <button type="button" class="btnDisable" disabled>고장 및 파손 접수</button>
                <button type="button" class="wishSub" onclick="window.location.href='../my/reviewWrite.html'">
                  리뷰 작성
                </button>
              </div>
            </div>
          </div>
          <div class="line"></div>
          <!-- 구독 완료 끝 -->
        </form>
        <div class="paging2">
          <a href=""><i class="bi bi-chevron-double-left"></i></a>
          <a href="" class="active">1</a>
          <a href="">2</a>
          <a href="">3</a>
          <a href="">4</a>
          <a href="">5</a>
          <a href=""><i class="bi bi-chevron-double-right"></i></a>
        </div>
      </div>
    </div>
  </div>
</div>

<script>
  function returnRequest() {
    if (confirm("남은 기간은 환불되지 않고 결제됩니다. 회수신청 하시겠습니까?") == true) {
      //확인
      document.form.submit();
    } else {
      //취소
      return;
    }
  }
</script>

<!-- ----------------------------------- content 끝 ----------------------------------- -->
<%@ include file="../main/footer.jsp"%>
